...
void char_code() {
	...
	my = readSoure(s_temp);
	int num = 0;  char* Code;
	char s[NUMBER];   int  w[NUMBER] = { 0 }; //用来存放统计好的字符集和频度集
	Stat(my, w, num);  //  统计字符频度对集
	int* w_new = new int[num];
	deletezero(s, w, num, w_new);
	tt = HuffmanBuild(s, w_new, num);    // 建huffman树
	tt->print();
	Code = new char[num];        //分配 编码结果的工作空间
...
{
	HuffTree<char>* ttree[NUMBER];//用字符集 s 和权值集w 新申请num棵树，令ttree的元素指向他们
	HuffTree<char>* tempTree;
	for (int i = 0; i < num; i++) {
		ttree[i] = new HuffTree<char>(s[i], w[i]);//tree leaf create
	}
	int j = num;
	while (j != 1)
	{
...
void getCodeMap(HuffNode<char>* root, string pastCode) {
	...
	if (!root->isLeaf())
	{
		...
		code[((LeafNode<char>*)root)->val()] = pastCode;
	}
}
...
void WriteToOutfp() {
	...
	cout << "文件压缩率为：" << (float)codeText.length() / (float)((my.length() - 1) * 8) << endl;
	fclose(Outfp);
}
...
void File_decode() {
	...
	FILE* result = fopen("f_result.txt", "w");
	if (result == NULL) {
		cout << "解码至txt文件打开失败" << endl;
		return;
	}
	unsigned int numtemp = 0;
	while (numtemp < my.length() - 1) {   //-1是因为my字符串在默认自动添加0作为标识，需将其减去
